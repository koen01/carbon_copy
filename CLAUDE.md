...
- **Sub-status names**: `models/sub_status.dart` is the official app's sub_status enum as published by the elegoo-web project (`src/types.ts`); `PrinterState.phase` uses it for the label after the state text (e.g. "PRINTING · AUTO LEVELING"). State itself is derived from `print_status.state` / `machine_status.status` first because the sub_status table alone is incomplete.
